...
def share(request, ref_id) :
	try:
		...
		ref_url = settings.BENGOK + str(join_obj.ref_id)
...
def home(request) :
	...
	if form.is_valid():
		...
		if created:
			...
			#add our friend who reffered us to join model or a related
			if not obj == None:
				new_join_old.friends = obj
			new_join_old.ip_addres = get_ip(request)
			new_join_old.save()
		#print all "friends" that joined as a result of owner main sharer email
